...
public class ColorPickerView extends View {
	...
	private boolean mTrackingCenter;//点击了中间的圆形区域
	private boolean mHighlightCenter;//从点击了圆环之后，移动
	...
	//算出两个值之间的指定比例的值公式为：value = p1 + p(p2-p1)
	private int ave(int s, int d, float p) {
		return s + java.lang.Math.round(p * (d - s));
	}

	//unit为触发点的到圆形的角度
	//一个区域值unit到颜色数组中找到其对应的色值区域
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		//判断当前触摸点的坐标是否在半径为CENTER_RADIUS的圆中
		...
		case MotionEvent.ACTION_MOVE:
			if (mTrackingCenter) {
				...
				//触发点的到圆形的角度，然后interpColor(mColors, unit)方法算出颜色
			} else {
